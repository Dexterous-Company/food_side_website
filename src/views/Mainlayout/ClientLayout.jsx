...
const reverseGeocode = async (lat, lng) => {
  try {
    ...
    const city =
      findComponent(["locality", "administrative_area_level_3"]) || "";
    ...
    const cleanedPostcode = postcode
      ? postcode.replace(/\s+/g, "").replace(/[^0-9]/g, "")
      : "";
    ...
    return {
      ...
      fromLocationDetailed:
        ...
        [road, city, state, cleanedPostcode].filter(Boolean).join(", "),
      city: city,
      state: state,
...
export default function ClientLayout({ children }) {
  ...
  const hideHeaderMobile =
    hideHeaderMobileRoutes.includes(pathname) || isDeliveryModalOpen;
  const hideFooterMobile =
    hideFooterMobileRoutes.includes(pathname) || isDeliveryModalOpen;
  ...
  return (
    <>
      {!hideHeaderRoutes.includes(pathname) && (
        <div
          className={`hidden sm:block ${hideHeader ? "invisible pointer-events-none" : ""}`}
        >
          <Header />
        </div>
      )}
      ...
      {!hideHeaderMobileRoutes.includes(pathname) && (
        <div
          className={`block sm:hidden ${hideHeaderMobile ? "invisible pointer-events-none" : ""}`}
        >
          <Header />
        </div>
      )}
      ...
      {!hideFooterRoutes.includes(pathname) && (
        <div
          className={`hidden sm:block ${hideFooter ? "invisible pointer-events-none" : ""}`}
        >
          <Footer />
        </div>
      )}
      ...
      {!hideFooterMobileRoutes.includes(pathname) && (
        <div
          className={`block sm:hidden ${hideFooterMobile ? "invisible pointer-events-none" : ""}`}
        >
          <MobileFooter />
        </div>
      )}
