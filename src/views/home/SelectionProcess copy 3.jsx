...
const SelectionProcess = () => {
  ...
  // Get the minimum allowed time (current time + 1 hour)
  const getMinAllowedTime = () => {
    const now = dayjs();
    return now.add(1, "hour");
  };

  // Disable times that are less than or equal to 1 hour from current time
  const disabledTime = () => {
    const now = dayjs();
    ...
    if (isToday) {
      return {
        disabledHours: () => {
          ...
          // Disable all hours before minHour
        ...
        disabledMinutes: (hour) => {
          // If hour is equal to minHour, disable minutes up to minMinute
          if (hour === minHour) {
            const disabledMinutesArray = [];
            // Disable minutes from 0 to minMinute (inclusive)
  ...
  // Custom validation when selecting time - ensures time is strictly after 1 hour from now
  const handleTimeChange = (time) => {
    ...
    // Check if selected time is BEFORE or EQUAL to minAllowedTime (1 hour from now)
    if (selectedDateTime.isBefore(minAllowedTime) || selectedDateTime.isSame(minAllowedTime)) {
        ...
        <div className="bg-white rounded-2xl shadow-md">
          ...
          <div className="hidden lg:block p-4">
            <div className="relative flex items-stretch border border-gray-200 rounded-xl bg-white ">
              ...
              {/* Time Picker with 1+ hour restriction */}
              <div className="border-l text-white border-gray-200 flex items-center px-3">
                <TimePicker
                  value={selectedTime}
          ...
          <div className="lg:hidden p-4">
            <div className="space-y-3">
              ...
              {/* Time Picker Mobile with 1+ hour restriction */}
              <div className="border border-gray-200 rounded-lg ">
                <TimePicker
                  value={selectedTime}
                  onChange={handleTimeChange}
