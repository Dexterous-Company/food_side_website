// components/Checkout/LoginPrompt.jsx (fixed - proper data handling)
"use client";
...
const LoginPrompt = ({ isDesktop, isUserAuth, userData, onLogin }) => {
  ...
  // Verify OTP - After verification, check if user exists
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Check if user exists (has name and email)
        const hasUserData = result.data && (result.data.name || result.data.email);
        ...
        if (hasUserData) {
          ...
          // New user - need to collect name and email
          setVerifiedMobileNumber(mobileNumber);
          setShowRegistrationForm(true);
          setShowOtpInput(false);
          setOtp("");
          showToast("OTP verified! Please complete your registration", false);
        }
      } else {
  ...
  const handleCompleteRegistration = async () => {
    ...
    try {
      ...
      if (createResult.success) {
        // Get the complete user data from response
      ...
      <div className={isDesktop ? "mb-6 p-6 bg-gray-50 rounded-2xl border border-gray-200" : "mb-6 p-6 bg-gray-50 rounded-2xl border border-gray-200"}>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">✓ Verified with OTP</p>
          </div>
